...
const CSS = {
  ...
  skipButton:
    ".videoAdUiSkipButton,.ytp-ad-skip-button,.ytp-ad-skip-button-modern,.ytp-skip-ad-button,.ytp-ad-skip-button-slot button,button[id^='skip-button']",
  // the player while it is playing an ad
  adShowingPlayer: "#movie_player.ad-showing,.html5-video-player.ad-showing",
  // the area showing the countdown to the skip button showing
  preSkipButton:
    ".videoAdUiPreSkipButton,.ytp-ad-preview-container,.ytp-preview-ad",
  // little x that closes banner ads
  closeBannerAd:
    ".close-padding.contains-svg,a.close-button,.ytp-ad-overlay-close-button",
...
const util = {
  ...
  getPath: (obj, path) =>
    obj == null
      ? null
      : path.length > 0
        ? util.getPath(obj[path.pop()], path)
        : obj,
};
...
function setupCancelDiv(ad) {
  ...
  if (skipText) {
    if (
      ["will begin", "will play", "plays soon"].some((snip) =>
        skipText.includes(snip),
      )
    )
      return;
    ...
    cancelDiv.textContent =
      (conf.muteAd ? "Un-mute & " : "") + "Cancel Auto Skip";
    cancelDiv.onclick = () => {
      ...
      if (conf.muteAd && muteButton && muteIndicator && isMuted(muteIndicator))
        muteButton.click();
    };
    ad.appendChild(cancelDiv);
  } else {
    util.log(
      "skip button area wasn't there for some reason.. couldn't place cancel button.",
    );
  }
}
...
const getMuteButton = () =>
  util.qq(CSS.muteButton).find((elem) => elem.offsetParent);
const getMuteIndicator = () =>
  util.qq(CSS.muteIndicator).find((elem) => elem.offsetParent);
  ...
  TICKS.push(
    ...
    waitForElems({
      ...
      onmatch: (ad) => {
        ...
        const muteIndicator = getMuteIndicator();
        if (!muteIndicator)
          return util.log("unable to determine mute state, skipping mute");
...
// Polls the player's "ad-showing" state instead of waiting for the skip button
// to be inserted: YouTube reuses the same skip button element across ads, so an
// insertion-based watcher only ever fires for the first ad. If clicking skip
// doesn't end the ad (or there is no skip button), jump the ad video to its end.
function watchAdPlayback() {
  let adStartedAt = 0;
  let skipClicked = false;
  return setInterval(() => {
    const player = util.q(CSS.adShowingPlayer);
    if (!player) {
      if (adStartedAt) util.log("ad finished");
      adStartedAt = 0;
      skipClicked = false;
      DONT_SKIP = false;
      return;
    ...
    if (!adStartedAt) {
      ...
      util.log("ad playing, skipping in", conf.secWaitVideo, "s");
    }
    if (DONT_SKIP || Date.now() - adStartedAt < conf.secWaitVideo * 1000)
      return;
    const btn = util.qq(CSS.skipButton).find((elem) => elem.offsetParent);
    if (btn && !skipClicked) {
    ...
    const video = util.q("video", player);
    if (
      video &&
      Number.isFinite(video.duration) &&
      video.currentTime < video.duration - 0.1
    ) {
      util.log("fast-forwarding ad to its end");
