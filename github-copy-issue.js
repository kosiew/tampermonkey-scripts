...
(function () {
  ...
  const IS_PULL_REQUEST_PAGE = window.location.href.match(
    /https:\/\/github\.com\/.*\/pull\/\d+/,
  );
  const IS_ISSUE_PAGE = window.location.href.match(
  ...
  function extractAllContent() {
    const pageTitle =
      document
        .querySelector("h1.gh-header-title, h1.title")
        ?.textContent?.trim() || "";
    const pageUrl = window.location.href;
    const repoUrl =
      pageUrl.match(/^(https:\/\/github\.com\/[^\/]+\/[^\/]+)/)?.[1] || "";
  ...
  function extractMainContent() {
    const container = document.querySelector(
      '[data-testid="issue-viewer-container"], .js-issue-markdown',
    );
    if (!container) return null;

    const markdownElement = container.querySelector(
  ...
  function extractComments() {
    ...
    commentElements.forEach((comment, index) => {
      const author =
        comment
          .querySelector(".author, .comment-author")
          ?.textContent?.trim() || "Unknown";
      const timestamp =
        comment
          .querySelector("relative-time, .timestamp")
      ...
      const permalink =
        comment.querySelector(
          'a[href*="#issuecomment-"], a[href*="#discussion_r"]',
        )?.href || "";

      const markdownBody = comment.querySelector(
        '[data-testid="markdown-body"], .comment-body',
      );
      if (markdownBody) {
  ...
  function processElementToMarkdown(element) {
    ...
    // Handle headers
    Array.from(clone.querySelectorAll("h1, h2, h3, h4, h5, h6")).forEach(
      (header) => {
